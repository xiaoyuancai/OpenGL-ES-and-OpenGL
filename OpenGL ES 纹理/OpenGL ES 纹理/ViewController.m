...
- (void)viewDidLoad
{
    ...
    //GLKTextureLoader类 用于将一个纹理图像加载到一个OPen ES纹理缓存中（GLKTextureLoader会自动调用glTextParameteri()方法来创建的纹理缓存设置OpenLG ES取样和循环模式）
    //textureInfo 封装了与刚创建的纹理缓存相关的信息，包括它的尺寸及它是否包含MIP贴图(此示例没有贴图)。
    GLKTextureInfo *textureInfo = [GLKTextureLoader textureWithCGImage:imageRef options:nil error:NULL];

    self.baseEffect.texture2d0.name = textureInfo.name;//纹理缓存名称
    self.baseEffect.texture2d0.target = textureInfo.target;//target指定被配置的纹理缓存的类型
